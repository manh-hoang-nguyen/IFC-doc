...
function buildingGenerator(startNum, model = {}, ownerHistory) {
  const { IfcBuildingGUID, name } = model;
  ...
  const L4 = `#${startNum + 3}= IFCBUILDING(${valueConverter(IfcBuildingGUID)},${valueConverter(
    ownerHistory,
  )},${valueConverter(name)},$,$,#${startNum + 2},$,'',.ELEMENT.,$,$,$);`;
